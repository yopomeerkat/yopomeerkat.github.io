function getInfo(initialInfo) {
	...
	var jsonInternalPromise = fetch(
		"https://api.etherscan.io/api?module=account&action=txlistinternal&address=" +
			initialInfo.address +
			"&startblock=" +
			((initialInfo.lastBlock || 0) + 1) +
		...
		.then(function(json) {
			var info = json.result.reduce(function(info, tr) {
				...
				if (val) {
					...
					info.sums.push(
						Math.round(info.sum / Math.pow(10, 16)) / 100
					);

					if (info.min == -1 || info.min > investment)
						info.min = investment;
					if (investment > info.max) info.max = investment;
				}
				return info;
			}, initialInfo);
			...
			return rate.then(function(ratejson) {
				...
				jsonInternalPromise.then(
					...
					function(e) {
						console.log(
							"Error fetching internal transactions: " + e
						);
						return info;
					}
				);
			...
			series: [
				{
					...
					data: info.sums.map(function(e, i) {
						return [info.dates[i], e];
					}),
					marker: {
						lineWidth: 2,
				...
				},
				{
					...
					type: "spline",
					name: "Users",
					data: info.nums.map(function(e, i) {
						return [info.dates[i], e];
					}),
					color: "#78ee06",
					enableMouseTracking: true,
...
function findAddress(address) {
	...
	if (investmentInfo.investors[addr]) return addr;

	for (var key in investmentInfo.investors) {
		if (key.indexOf(addr) === 0) return key;
	}
...
function updateDividents(investmentInfo, type) {
	...
	if (type !== "Month") {
		...
		for (var i = 0; i < investmentInfo.inv.length; ++i) {
			...
			divs +=
				inv.sum *
				(curTime - Math.max(inv.time, investmentInfo.gotTime || 0));
		}
...
function calcInvestment(resetTime) {
	...
	var text = findAddress(getLocalAccount()); //;localStorage.getItem("address") ;
	if (!text) {
		var inp = document.getElementById("inputInvestments");
		text = inp.value.trim().replace(/,/g, ".");
	}
			...
			inv: [
				{
					...
					time: resetTime
						? +new Date()
						: updateDividentsTimer.startTime
				}
			]
		};
